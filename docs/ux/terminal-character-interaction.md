...
| **Pty stub (focused, not pty-yet)**  | Halo is yellow-tinted (`--pixel-warning`) to signal "panel cannot show this terminal" — pairs with `TerminalPaneStub` hint. **Deferred** in v1: visual cue is low-value and the first-byte signal wasn't threaded into the renderer; the `TerminalPaneStub` hint alone covers it. |
