...
- `e2e/standalone/character-behaviors.spec.ts:56` — unnamed spawn shows its terminal name in the rail and overlay (Standalone / Character behaviors)
- `e2e/standalone/character-behaviors.spec.ts:101` — pty crash marks the character and restart clears it (Standalone / Character behaviors)
- `e2e/standalone/character-behaviors.spec.ts:133` — reload re-glyphs a crashed agent (Standalone / Character behaviors)
...
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:175` — an agent for the MAPPED folder takes a seat inside its area (Areas (multi-root) › seat preference (alpha → Engineering))
- `e2e/claude/hooks-off/areas-multiroot.spec.ts:206` — an agent for an UNMAPPED folder is not forced into the area (Areas (multi-root) › seat preference (alpha → Engineering))
...
### `@area:terminal` (4 tests)

- `e2e/standalone/terminal.spec.ts:35` — + Agent spawns a pty agent: character, band, mock invocation (Standalone / Terminal band)
- `e2e/standalone/terminal.spec.ts:56` — typed keystrokes reach the pty and echo back (Standalone / Terminal band)
- `e2e/standalone/terminal.spec.ts:85` — pty exit shows the Restart control and restart re-invokes claude (Standalone / Terminal band)
- `e2e/standalone/terminal.spec.ts:122` — a pane mounted AFTER the pty exited still shows the Restart control (Standalone / Terminal band)
