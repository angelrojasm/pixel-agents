/**
 * Unit tests for `desaturateSpriteData` — the saturation-cut pipeline behind
 * the crashed-character sprite treatment (Task 6 of the m1.5
 * character-behaviors slice). Ported from v2-orchestrator's
 * webview-ui/test/desaturate-sprites.test.ts (node:test → Vitest; assertion
 * style — node:assert/strict alongside vitest's `test` — mirrors
 * greeter.test.ts elsewhere in this suite).
 *
 * Run with: npm run test:webview -- test/desaturate-sprites.test.ts
 */
import assert from 'node:assert/strict';

import { test } from 'vitest';

import { desaturateSpriteData } from '../src/office/engine/desaturateSprites.js';

test('desaturateSpriteData leaves empty pixels untouched', () => {
  const sprite = [
