...
type CrashedHandler = (info: { code?: number; signal?: string }) => void;
type RestartedHandler = () => void;
...
interface AgentSubscribers {
  ...
  agentCrashed: Set<CrashedHandler>;
  agentRestarted: Set<RestartedHandler>;
}
...
type Handler<E extends keyof AgentSubscribers> = E extends 'ptyData'
  ...
  : E extends 'ptyExit'
    ...
    : E extends 'ptyScrollback'
      ? ScrollbackHandler
      : E extends 'agentCrashed'
        ? CrashedHandler
        : E extends 'agentRestarted'
          ? RestartedHandler
          : ActivityHandler;
 ...
 *
 * Ported from v2-orchestrator, extended with `agentCrashed`/`agentRestarted`
 * channels so the terminal pane can show crash state and clear on restart.
 */
export class PtyEventBus {
  ...
  private slot(agentId: number): AgentSubscribers {
    ...
    if (!s) {
      s = {
        ...
        ptyActivity: new Set(),
        agentCrashed: new Set(),
        agentRestarted: new Set(),
      };
  ...
  emitCrashed(agentId: number, info: { code?: number; signal?: string }): void {
    const s = this.agents.get(agentId);
    if (!s) return;
    for (const h of s.agentCrashed) h(info);
  }

  emitRestarted(agentId: number): void {
    const s = this.agents.get(agentId);
    if (!s) return;
