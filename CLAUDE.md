...
**Phase 2 — In progress**: Click an agent → terminal renders **inside the office** (xterm.js in the webview) instead of in VS Code's terminal strip. Extension spawns Claude via `node-pty` on the host, bridging stdin/stdout over `postMessage`. D1 (MessageSource refactor) and D2 (node-pty backend + xterm pane) shipped behind the in-app `usePtyTerminal` toggle. **Phase 2 follow-ups (queued, multi-iteration)**: terminal pane visual design (font, colors, padding, borders, tab/header treatment matching the pixel-art aesthetic), terminal QoL (resize/reflow, copy/paste, link handling, scrollback search, font size controls, focus behavior, keybinding conflicts with the panel chrome), and the **terminal ↔ character interaction layer** (how an agent's typing/idle/permission state in the pty maps onto the character's animation and bubbles, how clicking the character focuses the pty, how teammates/sub-agents are represented when their parent is pty-backed, hook-error surfacing in the panel vs. character overlay). Treat these as a rolling set of small changes rather than one big plan.
