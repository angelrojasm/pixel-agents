...
src/                          — Extension backend (Node.js, VS Code API)
  ...
  PixelAgentsViewProvider.ts   — WebviewViewProvider, full-screen WebviewPanel, multi-webview broadcast sink, message dispatch, asset loading, server lifecycle
...
webview-ui/src/               — React + TypeScript (Vite)
  ...
  components/
    ...
    SettingsModal.tsx          — Centered modal: sessions/layout/assets actions, sound/hooks/labels toggles, default terminal folder input, debug toggle
...
**Extension ↔ Webview**: `postMessage` protocol. Key messages: `openClaude`, `agentCreated/Closed`, `focusAgent`, `agentToolStart/Done/Clear`, `agentStatus`, `existingAgents`, `layoutLoaded`, `furnitureAssetsLoaded`, `floorTilesLoaded`, `wallTilesLoaded`, `saveLayout`, `saveAgentSeats`, `exportLayout`, `importLayout`, `settingsLoaded` (includes `externalAssetDirectories`, `defaultCwd`, `recentAgentFolders`), `setSoundEnabled`, `setDefaultCwd` (value: string), `addExternalAssetDirectory`, `removeExternalAssetDirectory` (field: `path`), `externalAssetDirectoriesUpdated` (field: `dirs`).

**Multi-webview broadcast**: `PixelAgentsViewProvider` maintains a `Set<Webview>` and a `broadcastSink: MessageSink` that fans posts to every registered webview. Every downstream module (`agentManager`, `fileWatcher`, `transcriptParser`, `assetLoader`, `timerManager`, `server/hookEventHandler`) takes `MessageSink | undefined` instead of `vscode.Webview | undefined`. This lets the side-panel `WebviewView` and a full-screen `WebviewPanel` coexist with synchronized state. The `MessageSink` interface is intentionally narrow (just `postMessage`) so it survives the Phase-3 transition out of VS Code — a WebSocket wrapper or equivalent slots in trivially.

**Full-screen panel**: Command `pixel-agents.openFullScreen` (also a `$(screen-full)` icon in the side-panel view title bar) opens a `WebviewPanel` in the editor area with `retainContextWhenHidden: true`. Same HTML, same message handler (`handleWebviewMessage`) as the side panel. On dispose, the panel is removed from the broadcast set. The side-panel view and full-screen panel are independent — both can be open at once and stay in sync via the broadcast sink. `openFullScreenPanel()` guards `reveal()` with a try/catch so a stale `fullScreenPanel` reference (VS Code disposed the panel but `onDidDispose` hadn't fired yet) falls through to creating a fresh panel instead of throwing "webview is disposed."

**One-agent-per-terminal**: Each "+ Agent" click → new terminal (`claude --session-id <uuid>`) → immediate agent creation → 1s poll for `<uuid>.jsonl` → file watching starts. The + Agent hover menu's "New agent…" form optionally sets a name (`openClaude.name` → `customTitle`, same slot as `/rename`) and a starting folder (`openClaude.folderPath`, `~` expanded + validated via `resolveRequestedCwd`; invalid paths fall through the normal cwd chain). Explicit folders are remembered in `config.json` under `pixel-agents.recentAgentFolders` (MRU, cap `RECENT_AGENT_FOLDERS_MAX`) and offered as quick-picks in the form.
...
**Catalog**: `furniture-catalog.json` with id, name, label, category, footprint, isDesk, canPlaceOnWalls, groupId?, orientation?, state?, canPlaceOnSurfaces?, backgroundTiles?. String-based type system (no enum constraint). Categories: desks, chairs, storage, electronics, decor, wall, misc. Wall-placeable items (`canPlaceOnWalls: true`) use the `wall` category and appear in a dedicated "Wall" tab in the editor. Asset naming convention: `{BASE}[_{ORIENTATION}][_{STATE}]` (e.g., `MONITOR_FRONT_OFF`, `CRT_MONITOR_BACK`). `orientation` is stored on `FurnitureCatalogEntry` and used for chair z-sorting and seat facing direction. Note: the work/rest seat rule keys off `category === 'electronics'`. External asset packs that ship monitors/computers under a different category won't classify those chairs as work seats; re-categorize via `asset-manager.html`.
...
- **Work vs rest seats**: position carries meaning. Character at a work seat (chair facing a computer) ⇒ working/thinking. Wandering or at a rest seat (couch) ⇒ idle. The overlay's "Working…" label derives from `isActive && no active tool`; position does the heavy visual lifting
- **Ghost-session prune on startup**: `restoreAgents` re-creates every persisted agent from `workspaceState`, including old external-adoptions from prior Watch-All sessions. `pruneForeignExternalsIfWatchAllOff()` runs once after restore: if Watch-All is off, any persisted `isExternal` agent whose `projectDir` isn't in the current workspace is removed + dismissed. Same ruleset as the explicit Watch-All toggle-off handler
- **Terminal cwd resolution**: `launchNewTerminal` tries, in order, the explicit folderPath arg, the first workspace folder, the user-set `defaultCwd` (via `resolveDefaultCwd()` — expands `~`, validates path), then `os.homedir()`. Non-existent `defaultCwd` values are silently skipped with a console warning; bad settings never block agent creation
- **Don't idle a character on a chair tile**: the chair tile carries the same visual meaning as "working." After a turn ends the FSM uses `STEP_OFF_PAUSE_MIN/MAX_SEC` (0.3–0.8s) to kick the first wander hop, and `isChairTile` guards any other IDLE transition that could leave a character stopped on a chair. Chair tiles themselves are excluded from the `walkableTiles` wander pool, so only the post-seat window needs this fix
- **Character display labels** resolve via `characterLabel(ch)` in `webview-ui/src/office/engine/characters.ts`. Priority: `customTitle ?? agentName ?? terminalName ?? 'Agent #${id}'`. `customTitle` comes from Claude's `/rename` (parsed in `transcriptParser` as a `custom-title` JSONL record; broadcast as `agentRenamed`). Used by `PanelHeader`, `ToolOverlay` nameplate, and `agentSummaries` in `App.tsx`. The team-role badge slot is purely for team roles (`isTeamLead` / `agentName`), NOT a fallback for `customTitle` — those concepts are kept separate
