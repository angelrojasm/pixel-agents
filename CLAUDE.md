...
**Phase 2 — Planned (on hold)**: Click an agent → terminal renders **inside the office** (xterm.js in the webview) instead of in VS Code's terminal strip. Extension spawns Claude via `node-pty` on the host, bridging stdin/stdout over `postMessage`. Removes the screen-split that splits the office from the terminal. UX decisions queued: per-agent terminal overlay anchoring, stack/hide affordance, focus management, copy/paste.
